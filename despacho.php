...
//echo $pagina;
	...
	$datos_despacho = $datos->consultar_remisiones_despacho($despacho);
	$despachador = $datos->detalle_usuario($datos->consultar_despachador($despacho));
	
?>

			<div id="titulo">
				<?php if ($_SESSION[nivel]==1) : ?>
							<h1>Despachado por: <?echo $despachador->fields["nombre_usuario"]?></h1>
				<button class="boton" onclick="window.location.href='principal?seccion=plantilla'">Cargar Archivo</button>
				<?php else : ?>
							<h1>Despachado por: <?echo $despachador->fields["nombre_usuario"]?></h1>
				<button class="boton" onclick="window.location.href='principal?seccion=plantilla'">Cargar Archivo</button>
				<?php endif; ?>

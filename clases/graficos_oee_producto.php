...
function grafica_oee_producto($indicadores,$valores)
{
	// Some data
	$datay=array(1,1,300);
	...
	if (file_exists('oee_producto.png'))
	{
		unlink('oee_producto.png');
	}
	// Finally stroke the graph
	$graph->Stroke('oee_producto.png');
}

  function pdf_grafico_oee_producto($sql,$inf)
    {
		//  echo $sql."<br>";
		include("conexion.php");
		$result=$conexion_pg->Execute($sql);
		...
		$pdf->Cell(80);
		$pdf->Cell(40,0,'GRAFICO OEE PARA:',0,0,'C',true);
		//$pdf->Ln();
		$pdf->Cell(5);
		$pdf->Cell(40,10,$result->fields['nombre'].", ".$inf,0,0,'C',true);
		...
		*/
		...
		$indicadores[3]="OEE";
		$valores=array();
		$valores[0]=round(floor($tdiponibilidad),0);
		$valores[1]=round(floor($tvelocidad),0);
		$valores[2]=round(floor($tcalidad),0);
		$valores[3]=round(floor($oee),0);
		$this->grafica_oee_producto($indicadores,$valores);
		$pdf->Image('oee_producto.png',10,30,250,150);
		$pdf->Output();
    }

...
//return true;
    ...
    public function iniciarTurno($turno) {
		...
		if($datos_turno['estadoTurno'] == 0){
			...
			$sql = "SELECT puerto FROM procesos WHERE id_proceso = $maquina";
			include("../clases/conexion.php");
			$result = $conexion_pg->Execute($sql);
			if ($result === false) die("fallo consulta");
			$conexion_pg->Close();
			
			$puerto = $result->fields["puerto"]; // puerto de la maquina
			
			$sql = "UPDATE conteos SET fecha_conteo = '$fecha', conteo_parada = '-1' WHERE id_puerto = '$puerto'";
			include("../clases/conexion.php");
			if ($conexion_pg->Execute($sql) === false) die("fallo consulta");
			$conexion_pg->Close();
		}
    ...
    public function registrarParada($maquina,$codigo) {
		...
		if($this->validarParada($codigo)){
			...
			if($paradaactual == "999"){
				...
				$sql = "UPDATE conteos c1 SET conteo_parada = (cast(c.conteo AS numeric) + p.unidades_salida) FROM procesos p INNER JOIN conteos c ON c.id_puerto = p.puerto WHERE p.id_proceso= '$maquina' AND c1.id_puerto = p.puerto";
				if ($conexion_pg->Execute($sql) === false) die("fallo consulta");
	...
	public function cerrarTurnoAutomatico($maquina, $maqturno){
		...
		$sql = "SELECT unidades_conteo
				FROM turnos
				WHERE id_proceso = $maquina and id_turno = $maqturno";
	...
	public function verificarCierre($maquina){
		...
		$sql = "SELECT tiempo_turno
				FROM turnos
				WHERE id_turno = $maqturno and id_proceso = $maquina";
